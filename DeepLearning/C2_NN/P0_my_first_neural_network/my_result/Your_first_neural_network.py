...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            # 囧，不细看还真没注意这个维度问题。。。
            hidden_inputs = np.dot(np.array(X, ndmin=2), self.weights_input_to_hidden) # signals into hidden layer
...
#             print(final_outputs)
            ...
            error = np.array(y, ndmin=2) - final_outputs # Output layer error is the difference between desired target and actual output.
            
            # TODO: Calculate the hidden layer's contribution to the error
            # hidden_error = np.dot(output_error_term, self.weights_hidden_to_output)
            hidden_error = np.dot(self.weights_hidden_to_output, error * 1).T
#             print(hidden_error)
            ...
            delta_weights_i_h += np.dot(np.array(X, ndmin=2).T, hidden_error_term)
...
# 
# #### 请将你的答案填写在下方
# 虽然没有对比很难评估个人的训练效果好坏，但就从我个人的小对比来看已经可以发现很多问题。首先，隐藏层的神经元个数越多，模型的表现能力越强这是无用质疑的，同时神经元个数的增加势必也会加大计算复杂度。所谓为了发现更好的模型难免需要不断的进行测试。本人一共进行了6次测试，测试结果如下所示。可见模型复杂度适度就好，只要模型表现能力足够，设置一个较好的学习率，剩下就是时间问题。
# 
# iterations = 10000 
# 
# learning_rate = 0.05 
